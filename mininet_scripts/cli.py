...
class CLI( Cmd ):
    ...
    def do_iperf3multi( self, line ):
        """Multi iperf3 TCP test between nodes"""
        args = line.split()
        if len(args) == 1:
            period = args[ 0 ]
            err = False
            self.mn.iperf3Multi(float(period))
        else:
            error('invalid number of args: iperf3multi <period> \n' +
                  '<period> example (in seconds): 60\n')

    def do_iperf3singleTCP( self, line ):
        """Single iperf3 TCP test between nodes"""
        args = line.split()
        if len(args) == 3:
            client = args[0]
            server = args[1]
            period = args[2]
            err = False
            self.mn.iperf3singleTCP([client,server],float(period))
        else:
            error('invalid number of args: iperf3singleTCP <client> <server> <period> \n' +
                  '<period> example (in seconds): 60\n')
