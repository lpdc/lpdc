...
class Mininet( object ):
    ...
    def iperf_single( self,hosts=None, udpBw='10M', period=5, port=5001, path='/home/iperf3multi'):
        ...
        bwArgs = '-b ' + udpBw + ' '
        print "***start server***"
        server.cmd( iperfArgs + '-s' + ' > ' + path + '/server' + filename + '&')
        print "***start client***"
        client.cmd(
    ...
    #for iperf3multi
    def iperf3_tcp( self,hosts=None, period=5, port=5201, path='/home/iperf3singleTCP'):
        """Run iperf3 between two hosts using TCP.
           hosts: list of hosts; if None, uses opposite hosts
           returns: results two-element array of server and client speeds"""
        if not hosts:
            return
            ...
            assert len( hosts ) == 2
        client, server = hosts
        #filename = client.name[1:] + '.out' #Recover only the number from position index 1 and above
        output( '\n\n***Iperf3: testing bandwidth between ' )
        output( "%s and %s" % ( client.name, server.name ) )
        
        iperfArgs = 'iperf3 '
        
        bwArgs = ' '
        output('\n***start server: %s' % server.name)
        server.cmd( iperfArgs + '-s -p ' + str(port) + ' -1 -f M --forceflush > ' + path + '/server_' + server.name + '&')
        output('\n***start client: %s' % client.name)
        client.cmd(
            iperfArgs + '-t '+ str(period) + ' -c ' + server.IP() +
            ' -p ' + str(port) + ' -R -f M --forceflush' + 
            ' > ' + path + '/client_' + client.name +'&')

        #TODO: run pingall to verify connections first
    def iperf3Multi(self, period=60, path='/home/iperf3multi'):
        base_port = 5201
        ...
        for i in xrange(0, _len):
            ...
            for j in xrange(0, _len):
                server = host_list[j]
                if( server != client ):
                    self.iperf3_tcp(hosts = [client, server], period= period, port=base_port, path=path)
                    sleep(.05)
                    base_port += 1
                    #self.hosts[0].cmd('ping -c10'+ self.hosts[-1].IP() + ' > /home/muzi/log/delay.out')
        #Note: a delay happens to flush the buffer of the generate output files, but they are created =)
        sleep(1+period)
        output('\nDone. Results were written in: ' + path + '\n')

    #For line command
    def iperf3singleTCP( self,hosts=None, period=5, port=5201, path='/home/iperf3singleTCP'):
        """Run iperf3 between two hosts using TCP.
           hosts: list of hosts; if None, uses opposite hosts
           returns: results two-element array of server and client speeds"""
        if not hosts:
        ...
        #iperf3singleTCP h1 h2 60, but 'h1' is a string provided in the cli.
        #Discover the hosts from the strings provided in the cli
        hosts_list = [h for h in self.hosts]
        for i in hosts_list:
            if i.name == hosts[0]:
                client = i
            if i.name == hosts[1]:
